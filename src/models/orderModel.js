...
async function createOrder({ user_id, total_amount, payment_status, payment_ref, shipping }) {
...
async function insertOrderItems(order_id, items) {
  ...
  const sql = `INSERT INTO order_items (order_id, product_id, product_name, quantity, unit_price, subtotal) VALUES ?`;
...
async function listOrdersByUser(user_id, { offset = 0, limit = 20 }) {
  const rows = await query(`SELECT * FROM orders WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?`, [user_id, limit, offset]);
  const [{ count }] = await query(`SELECT COUNT(*) AS count FROM orders WHERE user_id = ?`, [user_id]);
  return { rows, count };
}
...
async function adminListOrders({ status, payment_status, offset = 0, limit = 20 }) {
  const where = [];
  const params = [];
  if (status) { where.push('status = ?'); params.push(status); }
  if (payment_status) { where.push('payment_status = ?'); params.push(payment_status); }
  const whereSql = where.length ? 'WHERE ' + where.join(' AND ') : '';
  const rows = await query(`SELECT * FROM orders ${whereSql} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
  const [{ count }] = await query(`SELECT COUNT(*) AS count FROM orders ${whereSql}`, params);
  return { rows, count };
}
...
async function adminStats() {
  const revRows = await query(`SELECT COALESCE(SUM(total_amount),0) AS total_revenue FROM orders WHERE payment_status = 'paid'`);
