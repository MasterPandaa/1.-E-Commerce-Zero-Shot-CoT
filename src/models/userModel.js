...
async function listUsers({ offset = 0, limit = 20 }) {
  const rows = await query(`SELECT id, name, email, role, is_active, created_at FROM users ORDER BY id DESC LIMIT ? OFFSET ?`, [limit, offset]);
...
async function findValidPasswordReset({ token_hash }) {
  const rows = await query(`SELECT * FROM password_resets WHERE token_hash = ? AND used_at IS NULL AND expires_at > NOW() LIMIT 1`, [token_hash]);
  return rows[0] || null;
}
