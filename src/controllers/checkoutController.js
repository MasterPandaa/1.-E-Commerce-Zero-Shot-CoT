...
exports.checkout = async (req, res, next) => {
  ...
  try {
    ...
    for (const k of [
      ...
      if (!shipping[k]) {
        return res.status(422).json({ message: `Missing field: ${k}` });
      }
    }
    ...
    for (const it of items) {
      ...
      if (!p || p.stock < it.quantity) {
        return res
          .status(400)
          .json({ message: `Insufficient stock for ${it.product_name}` });
      }
    }
    ...
    await conn.execute("UPDATE carts SET status = 'ordered' WHERE id = ?", [
      cart.id,
    ]);
    ...
    res
      .status(201)
      .json({ order_id, payment_status, payment_ref, total_amount });
  } catch (err) {
    try {
      await conn.rollback();
    } catch (_) {}
