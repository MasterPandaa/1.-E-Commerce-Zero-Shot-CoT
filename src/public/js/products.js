...
async function loadProducts(page = 1) {
  ...
  list.innerHTML = resp.data.map(p => `
    <div class="card">
  ...
  `).join('');
  list.querySelectorAll('.addCart').forEach(btn => btn.addEventListener('click', async () => {
    try {
      await api('/cart/items', { method: 'POST', body: JSON.stringify({ product_id: Number(btn.dataset.id), quantity: 1 }) });
      alert('Added to cart');
