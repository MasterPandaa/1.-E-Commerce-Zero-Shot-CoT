...
async function loadOrders() {
  ...
  div.innerHTML = out.data.map(o => `<div class="card">#${o.id} - ${o.status} - $${o.total_amount}</div>`).join('');
}

async function loadUsers() {
  const out = await api('/admin/users?limit=20');
  const div = document.getElementById('users');
  div.innerHTML = out.data.map(u => `<div class="card">${u.id} - ${u.email} (${u.role})</div>`).join('');
}
...
async function listAdminProducts() {
  ...
  `).join('');
  div.querySelectorAll('.del').forEach(btn => btn.addEventListener('click', async () => {
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    await requireAdmin();
    await Promise.all([loadStats(), loadOrders(), loadUsers(), loadCategories(), listAdminProducts()]);
    ...
    if (form) {
      form.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          fd.append('category_id', document.getElementById('p_category').value || '');
